...
public class RecipeHandlerRollingMachineShapeless extends RecipeHandlerRollingMachine {
    ...
    private static int[][] INPUTS = new int[][] { { 0, 0 }, { 1, 0 }, { 0, 1 }, { 1, 1 }, { 0, 2 }, { 1, 2 }, { 2, 0 },
            { 2, 1 }, { 2, 2 } };
        ...
        public void setIngredients(List<?> items) {
            ...
            for (int ingred = 0; ingred < items.size(); ingred++) {
                ...
                PositionedStack stack = new PositionedStack(
                        item,
                        25 + RecipeHandlerRollingMachineShapeless.INPUTS[ingred][0] * 18,
                        6 + RecipeHandlerRollingMachineShapeless.INPUTS[ingred][1] * 18);
                stack.setMaxSize(1);
    ...
    private CachedRollingMachineShapelessRecipe getCachedRecipe(IRecipe irecipe, boolean genPerms) {
        ...
        if (irecipe instanceof ShapelessRecipes) {
            recipe = new CachedRollingMachineShapelessRecipe(
                    ((ShapelessRecipes) irecipe).recipeItems,
                    ((ShapelessRecipes) irecipe).getRecipeOutput());
