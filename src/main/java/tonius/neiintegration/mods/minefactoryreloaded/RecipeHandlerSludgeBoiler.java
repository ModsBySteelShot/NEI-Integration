...
public class RecipeHandlerSludgeBoiler extends RecipeHandlerBase {
    ...
    public class CachedSludgeBoilerRecipe extends CachedBaseRecipe {
        ...
        public CachedSludgeBoilerRecipe(ItemStack output, int weight) {
            this.sludgeInput = new PositionedFluidTank(
                    FluidRegistry.getFluidStack("sludge", sludgePerOperation),
                    4000,
                    new Rectangle(111, 2, 16, 60),
                    RecipeHandlerSludgeBoiler.this.getGuiTexture(),
                    new Point(176, 0));
    ...
    public void loadAllRecipes() {
        for (Item drop : drops) {
            if (drop instanceof WeightedRandomItemStack) {
                this.arecipes.add(
                        new CachedSludgeBoilerRecipe(((WeightedRandomItemStack) drop).getStack(), drop.itemWeight));
            }
        }
    }
